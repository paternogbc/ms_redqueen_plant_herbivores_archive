...
tax <- read.csv("data/processed/study_species_list_taxonomy.csv")
trees <- read.tree("data/processed/phylogentic_trees_300_s2.tre")
...
saveRDS(object = reg_plots, file = "output/temp/main_regression_plots.RDs")

# 5. Phylogentic tree--------------------------------------------------------------
circ <- ggtree(t, layout = "fan", open.angle = 25, color = gray(.5))
circ

d1 <- d %>% select(Maleness = maleness)
d2 <- d %>% mutate(
...
p2
pout <- 
  gheatmap(p2, d2, colnames = T, font.size = 3.5,
           colnames_position = "top",
           colnames_angle = 80,
           colnames_offset_y = 5,
           offset=15, width = .1) +
...
pout + 
  geom_cladelabel(node = nods[[1]], label = names(nods[1]), angle = 23, color = gray(.3), 
                  offset=38, offset.text = 9, hjust = .5) +
...
# save plot
ggsave(filename = "output/figures/figure_1.png", 
       plot = out, width = 178, height = 247, units = "mm")
ggsave(filename = "output/figures/figure_1.pdf", 
       plot = out, width = 178, height = 247, units = "mm")
